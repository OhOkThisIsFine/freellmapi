...
anthropicRouter.post('/messages', async (req: Request, res: Response) => {
  ...
  await runFallbackLoop({
    ...
    route: () => routeRequest(estimatedTotal, state.skipKeys.size > 0 ? state.skipKeys : undefined, preferredModel, hasImage, wantsTools, state.skipModels.size > 0 ? state.skipModels : undefined, familyPoolChain),
